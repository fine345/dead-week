...
func _gui_input(event: InputEvent) -> void:
	if event is InputEventScreenTouch:
		...
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed and _is_inside_base(event.position):
			active = true
			pointer_id = 0
			_update_from_position(event.position)
			accept_event()
...
func _is_inside_base(screen_position: Vector2) -> bool:
	var local_position: Vector2 = screen_position - base.get_global_position()
